...
static int gip_headset_start_audio(struct gip_headset *headset)
{
	...
	/* all standalone clients require authentication */
	if (!gip_is_accessory(client)) {
...
static void gip_headset_power_on(struct work_struct *work)
{
	...
	/* accessories have limited capabilities */
	if (gip_is_accessory(client))
		return;
	...
	err = gip_init_led(&headset->led, client);
	if (err)
		dev_err(&client->dev, "%s: init LED failed: %d\n",
			__func__, err);
}
